...
class CustomAudioSourceMain: BaseViewController {
    ...
    var simplePlayer = SimpleAudioPlayer()
//    var exAudio: ExternalAudio = ExternalAudio.shared()
    ...
    override func viewDidLoad(){
        ...
        let sampleRate:UInt = 48000, channel:UInt = 2
        ...
        agoraKit.enableExternalAudioSource(
            withSampleRate: sampleRate,
            channelsPerFrame: channel
        )

        // setup external audio source
        simplePlayer.setupExternalAudio(withAgoraKit: agoraKit,
                                        sampleRate: uint(sampleRate),
                                        channels: uint(channel))
...
extension CustomAudioSourceMain: AgoraRtcEngineDelegate {
    ...
    func rtcEngine(_ engine: AgoraRtcEngineKit, didJoinChannel channel: String, withUid uid: UInt, elapsed: Int) {
        ...
        if let url = Bundle.main.url(forResource: "leva-eternity-149473", withExtension: "mp3") {
            simplePlayer.loadSongURL(url)
        }
